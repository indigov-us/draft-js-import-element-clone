...
var ContentGenerator = /*#__PURE__*/function () {
  ...
  _createClass(ContentGenerator, [{
    ...
    value: function processImageElement(element) {
      ...
      var entityKey = this.createEntity(_draftJsUtils.ENTITY_TYPE.IMAGE, data);

      if (parent.tagName === 'figure') {
        parent.entityStack.push(entityKey); // self closing tag
      ...
      var atomicBlock = {
        tagName: 'figure',
        textFragments: [],
        type: _draftJsUtils.BLOCK_TYPE.ATOMIC,
        styleStack: [NO_STYLE],
    ...
    value: function processNode(node) {
      if (node.nodeType === _syntheticDom.NODE_TYPE_ELEMENT) {
        ...
        if (_tagName === 'img') {
          this.processImageElement(_element);
